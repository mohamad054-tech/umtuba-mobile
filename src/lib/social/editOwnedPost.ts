 ...
 * Media path switches only after a validated revision. RLS/owner filter remains.
 */
...
export const viewerMaySeeEditControl = viewerMayEditPost;
  ...
  if (!post.article_id && title && body) {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        published_at:
          post.media_status === "draft" ? null : new Date().toISOString(),
      })
      .select("id")
      .single();
    if (error || !data?.id) {
...
> {
  ...
  if (
    !media ||
    media.kind === "none" ||
    media.kind === "replace_image" ||
    media.kind === "add_image" ||
...
export async function updatePostForOwner(
  ...
  return {
    ...
    mediaSwitched:
      videoRevision.switched ||
      mediaKind === "replace_image" ||
      mediaKind === "add_image" ||
      mediaKind === "remove_image",
  };
}

/**
 * Persist Create/editor VIDEO_EDIT_STATE onto the same owned post.
 * Original video_path is never rewritten. Cancel is a no-op (do not call).
 */
export async function saveOwnedVideoEditState(
  supabase: SupabaseClient,
  userId: string,
  postId: number,
  ...
  const durationMs = post.media_duration_ms ?? editState.trimEndMs;
  const trim = normalizeTrimRange(
    { inMs: editState.trimStartMs, outMs: editState.trimEndMs },
    durationMs
  );
  const mediaPipeline = serializeEditIntoMediaPipeline(
    mergeMediaPipelineEdit(post.media_pipeline, {
      trim,
