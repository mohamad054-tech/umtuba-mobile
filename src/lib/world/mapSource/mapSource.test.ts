...
describe("StreetMapSource", () => {
  it("is available with OpenFreeMap liberty style", () => {
    const source = createStreetMapSource();
    expect(source.id).toBe(STREET_MAP_SOURCE_ID);
    expect(source.kind).toBe("street");
    expect(source.label).toBe("Streets");
    expect(source.isAvailable()).toBe(true);
    expect(source.getStyleUrl()).toBe(STREET_MAP_STYLE_URL);
    expect(isWorldMapSourceAvailable(source)).toBe(true);
  });
});

describe("SatelliteMapSource", () => {
  it("is available with inline ESRI style data URI", () => {
    const source = createSatelliteMapSource();
    expect(source.id).toBe(SATELLITE_MAP_SOURCE_ID);
    expect(source.kind).toBe("satellite");
    expect(source.isAvailable()).toBe(true);
    expect(source.getStyleUrl()).toBe(SATELLITE_MAP_STYLE_URL);
    expect(source.getStyleUrl()).toMatch(/^data:application\/json,/);
    expect(isWorldMapSourceAvailable(source)).toBe(true);
  });
});

describe("placeholder map sources", () => {
  it("Terrain is unavailable fail-closed", () => {
    const source = createTerrainMapSource();
...
describe("MapSourceRegistry", () => {
  it("lists defaults with Demo, Street, Satellite available", () => {
    const registry = createDefaultMapSourceRegistry();
    expect(registry.list()).toHaveLength(4);
    expect(registry.listAvailable()).toHaveLength(3);
    const availableIds = registry.listAvailable().map((s) => s.id);
    expect(availableIds).toContain(DEMO_MAP_SOURCE_ID);
    expect(availableIds).toContain(STREET_MAP_SOURCE_ID);
    expect(availableIds).toContain(SATELLITE_MAP_SOURCE_ID);
    const resolved = registry.resolve();
    expect(resolved?.id).toBe(DEMO_MAP_SOURCE_ID);
    expect(resolved?.getStyleUrl()).toBe(DEMO_MAP_STYLE_URL);
  });

  it("resolves preferred Street and Satellite when available", () => {
    const registry = createDefaultMapSourceRegistry();
    expect(registry.resolve(STREET_MAP_SOURCE_ID)?.id).toBe(STREET_MAP_SOURCE_ID);
    expect(registry.resolve(SATELLITE_MAP_SOURCE_ID)?.id).toBe(
      SATELLITE_MAP_SOURCE_ID
    );
  });
  ...
  it("preferred unavailable id falls back to first available", () => {
    const registry = createDefaultMapSourceRegistry();
    const resolved = registry.resolve("world-map-source-terrain");
    expect(resolved?.id).toBe(DEMO_MAP_SOURCE_ID);
  });
...
describe("Runtime setMapSourceId", () => {
  it("switches street → satellite without resetting camera", async () => {
    const controller = createWorldRuntimeController({ yieldMs: 0 });
    await controller.start();
    const renderer = controller.getRendererAdapter();
    if (!isMapLibreRendererAdapter(renderer)) {
    ...
    controller.applyCameraControl("zoom_in");
    const cameraBefore = renderer.getSessionCamera();

    expect(controller.setMapSourceId(STREET_MAP_SOURCE_ID)).toBe(true);
    expect(controller.getSelectedMapSource()?.id).toBe(STREET_MAP_SOURCE_ID);
    expect(renderer.getStyleUrl()).toBe(STREET_MAP_STYLE_URL);
    expect(renderer.getSessionCamera()).toEqual(cameraBefore);

    expect(controller.setMapSourceId(SATELLITE_MAP_SOURCE_ID)).toBe(true);
    expect(controller.getSelectedMapSource()?.id).toBe(SATELLITE_MAP_SOURCE_ID);
    expect(renderer.getStyleUrl()).toBe(SATELLITE_MAP_STYLE_URL);
    expect(renderer.getSessionCamera()).toEqual(cameraBefore);
    expect(renderer.getMountGeneration()).toBeGreaterThan(1);
  });

  it("preserves selection and layer visibility across switch", async () => {
    const controller = createWorldRuntimeController({ yieldMs: 0 });
    await controller.start();
    ...
    const citiesVisibleBefore =
      renderer.getLayerAdapter().isLayerVisible("cities");
    const markerCountBefore = renderer.getPlaceMarkers().length;
    expect(markerCountBefore).toBeGreaterThan(0);

    expect(controller.setMapSourceId(SATELLITE_MAP_SOURCE_ID)).toBe(true);
    expect(renderer.getSelectedPlaceMarkerId()).toBe("place-amman");
    expect(renderer.getLayerAdapter().isLayerVisible("cities")).toBe(
      citiesVisibleBefore
    );
    expect(renderer.getPlaceMarkers().length).toBe(markerCountBefore);
    expect(controller.getViewState().mapSources.find((s) => s.active)?.id).toBe(
      SATELLITE_MAP_SOURCE_ID
    );
  });

  it("fail-closed: unavailable source falls back to Demo without crash", async () => {
    const brokenSatellite: WorldMapSource = {
      id: "world-map-source-satellite",
      kind: "satellite",
    ...
    const controller = createWorldRuntimeController({
      yieldMs: 0,
      mapSourceRegistry: registry,
    });
    await controller.start();

    expect(() => controller.setMapSourceId(SATELLITE_MAP_SOURCE_ID)).not.toThrow();
    expect(controller.getSelectedMapSource()?.id).toBe(DEMO_MAP_SOURCE_ID);
    expect(controller.getRendererAdapter().isBound()).toBe(true);
  });

  it("updates attribution in view state after switch", async () => {
    const controller = createWorldRuntimeController({ yieldMs: 0 });
    await controller.start();

    controller.setMapSourceId(STREET_MAP_SOURCE_ID);
    expect(controller.getViewState().attribution).toMatch(/OpenFreeMap/i);

    controller.setMapSourceId(SATELLITE_MAP_SOURCE_ID);
    expect(controller.getViewState().attribution).toMatch(/Esri/i);
  });
});

describe("MapLibre setStyleUrl", () => {
  it("bumps mountGeneration and preserves sessionCamera", () => {
    const adapter = createMapLibreRendererAdapter({
      styleUrl: DEMO_MAP_STYLE_URL,
    });
    adapter.mount();
    adapter.getCameraAdapter().zoomIn();
    const cameraBefore = adapter.getSessionCamera();
    const genBefore = adapter.getMountGeneration();

    expect(adapter.setStyleUrl(STREET_MAP_STYLE_URL)).toBe(true);
    expect(adapter.getStyleUrl()).toBe(STREET_MAP_STYLE_URL);
    expect(adapter.getMountGeneration()).toBe(genBefore + 1);
    expect(adapter.getSessionCamera()).toEqual(cameraBefore);
    expect(adapter.isStyleReady()).toBe(false);
  });

  it("returns true when same URL and rejects empty", () => {
    const adapter = createMapLibreRendererAdapter({
      styleUrl: DEMO_MAP_STYLE_URL,
    });
    expect(adapter.setStyleUrl(DEMO_MAP_STYLE_URL)).toBe(true);
    expect(adapter.setStyleUrl("  ")).toBe(false);
  });
});
