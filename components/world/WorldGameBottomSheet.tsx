...
type WorldGameBottomSheetProps = {
  sheet: WorldGameSheetState | null;
  bottomInset?: number;
  onClose?: () => void;
};

export function WorldGameBottomSheet({
  sheet,
  bottomInset = 0,
  onClose,
  ...
  return (
    <View
      ...
      accessibilityLabel={`Game details for ${sheet.gameName}`}
    >
      <View style={styles.handle} accessibilityElementsHidden />
      <View style={styles.header}>
        <View style={styles.headerText}>
          <Text style={styles.title}>{sheet.gameName}</Text>
          <Text style={styles.subtitle}>
            {sheet.categoryLabel} · {sheet.cityName}
          </Text>
        </View>
        {onClose ? (
          <Pressable
